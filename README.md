...
Finally, for the model part, I used a simple ResNet50. The reason I didn't use a more complex model is that I believed that if the method described in the paper is effective, even if the initial source score is low, both PL and PL+VAT scores would gradually increase. However, the experimental results showed that they did not improve over time.  
